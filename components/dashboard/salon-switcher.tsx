...
export function SalonSwitcher({ salons, activeSalonId, allowAll = true }: { salons: SalonOption[]; activeSalonId: string | null; allowAll?: boolean }) {
  ...
  return (
    <label className="block">
      ...
      <select
        value={activeSalonId ?? (allowAll ? "all" : salons[0]?.id)}
        disabled={pending}
      ...
      >
        {allowAll ? <option value="all" className="text-salon-ink">كل الفروع</option> : null}
        {salons.map((salon) => (
