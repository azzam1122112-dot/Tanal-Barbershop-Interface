...
يحتوي المشروع على `public/manifest.webmanifest` باسم "حلاق تنال" وأيقونات محلية داخل `public/icons`. لا يوجد service worker أو cache offline للصفحات المحمية في هذه المرحلة، لتجنب تخزين بيانات العملاء أو الجلسات محليًا.
...
- حلاق تنال - واجهة تنال للحلاقة الرجالية: `/barber/login`
