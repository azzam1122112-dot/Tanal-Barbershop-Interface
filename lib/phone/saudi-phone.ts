...
/**
 * يحوّل الشكل المعياري `+9665XXXXXXXX` إلى الشكل المحلي `05XXXXXXXX`.
 *
 * **عقدان مختلفان عمدًا:** طبقة الهوية العالمية تخزّن E.164 لأن مفتاح تفرّدها
 * عابر للبلدان، وسجل العميل داخل المؤسسة يخزّن الشكل المحلي لأن كل ما بُني عليه
 * — البحث في شاشة الحلاق، وروابط واتساب، والإيصالات — يتوقّعه هكذا. التحويل هنا
 * في موضع واحد بدل أن يتكرر في كل مسار يُنشئ عميلًا.
 *
 * **النسخ ليس توثيقًا:** الرقم يُنسخ كبيانات تواصل، ولا يجعل الرقم مُثبتًا ولا
 * مملوكًا عالميًا. `phoneVerifiedAt` يبقى فارغًا.
 */
export function toSaudiLocalPhone(input: string) {
  return `0${toSaudiE164(input).slice(4)}`;
}
