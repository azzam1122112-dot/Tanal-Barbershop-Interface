...
import { getBarberPerformanceReport } from "@/lib/reports/dashboard-reports";
...
export async function GET(request: Request) {
  ...
  const barbers = await getBarberPerformanceReport(prisma, { ...getReportFiltersFromUrl(new URL(request.url)), organizationId, salonIds });
