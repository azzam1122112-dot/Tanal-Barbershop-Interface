...
export default async function DashboardCustomersPage({
  ...
  return (
    <DashboardShell title="العملاء" description="بحث سريع في العملاء، متابعة النقاط والزيارات، والتحكم بتفضيل رسائل واتساب.">
        ...
        <TablePanel>
          <div className="hidden grid-cols-[1fr_150px_80px_80px_105px_155px_115px_125px] gap-3 border-b border-salon-line px-4 py-3 text-sm font-bold text-salon-charcoal lg:grid">
            <span>الاسم</span>
            <span>الجوال</span>
            <span>النقاط</span>
            <span>الزيارات</span>
          ...
          <div className="divide-y divide-salon-line">
            {rows.map((customer) => (
              <div
                key={customer.id}
                className="grid grid-cols-2 gap-x-4 gap-y-3 px-4 py-4 text-sm lg:grid-cols-[1fr_150px_80px_80px_105px_155px_115px_125px] lg:items-center lg:gap-3 lg:py-3"
              >
                ...
                <Cell label="واتساب"><CustomerWhatsAppToggle customerId={customer.id} initialTransactionalOptIn={customer.whatsappTransactionalOptIn} initialMarketingOptIn={customer.whatsappMarketingOptIn} initialConsentSource={customer.whatsappConsentSource} /></Cell>
                <Cell label="تاريخ الإنشاء">{formatDate(customer.createdAt)}</Cell>
                <Cell label="إجراء" className="col-span-2 lg:col-span-1">
                  <div className="grid gap-2">
                    <ManagerRewardButton customerId={customer.id} customerName={customer.name} />
                    <CustomerPortalLink
