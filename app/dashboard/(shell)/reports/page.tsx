  ...
  return (
    <DashboardShell title="التقارير المالية والتشغيلية" description="قراءة واضحة للدخل، الأداء، الخصومات، العملاء، وحركة الخدمات حسب الفترة والحلاق وطريقة الدفع.">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard label="الزيارات" value={formatNumber(revenue.visitsCount)} />
          <StatCard label="عملاء جدد / عائدون" value={`${revenue.newCustomersCount} / ${revenue.returningCustomersCount}`} />
          <StatCard label="النقاط المكتسبة" value={formatNumber(revenue.pointsEarned)} />
          <StatCard label="النقاط المستبدلة" value={formatNumber(revenue.pointsRedeemed)} />
          <StatCard label="مكافآت مستخدمة" value={formatNumber(revenue.rewardRedemptionsCount)} />
          <StatCard label="حملات مستخدمة" value={formatNumber(revenue.campaignRedemptionsCount)} />
        </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-2">
          ...
          <ReportSection title="الخصومات">
            <div className="grid gap-3 p-4 sm:grid-cols-2">
              ...
              <StatCard label="استخدام المكافآت" value={formatNumber(discounts.rewardRedemptionsCount)} />
              <StatCard label="استخدام الحملات" value={formatNumber(discounts.campaignRedemptionsCount)} />
            </div>
              ...
              <tbody className="divide-y divide-salon-line">
                {customers.topCustomers.slice(0, 12).map((row) => (
                  <tr key={row.customer.id}>
                    ...
                    <Cell>{row.visitsCount}</Cell>
                    <Cell>{formatMoney(row.netAmount)}</Cell>
                    <Cell>{row.lastVisitAt ? formatDate(row.lastVisitAt) : "-"}</Cell>
...
function ReportSection({ title, children }: { title: string; children: React.ReactNode }) {
  // السحب على المحتوى لا على اللوح كله — وإلا انزلق عنوان القسم مع الجدول.
  return (
