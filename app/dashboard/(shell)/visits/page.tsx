...
export default async function DashboardVisitsPage({
  ...
  const { orgWhere, salonWhere } = dashboardScope(session);
  ...
  const orgFilter = orgWhere;
  const [totalVisits, barbers, rewardRules, campaigns] = await Promise.all([
    prisma.visit.count({ where }),
    prisma.barber.findMany({ where: { ...orgFilter, ...salonWhere }, orderBy: { name: "asc" } }),
  ...
  return (
    <DashboardShell title="الزيارات" description="سجل سريع قابل للتصفية والتصحيح.">
        ...
        <div className="mt-6 flex flex-col gap-3 rounded-xl border border-salon-line bg-white/80 px-4 py-3 text-sm font-bold text-salon-charcoal shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <span>{formatNumber(totalVisits)} زيارة</span>
          <span>يعرض {formatNumber(firstItem)} - {formatNumber(lastItem)}</span>
        </div>

        {/* لوح عادي لا `TablePanel`: السجل يبدّل بنفسه بين بطاقات على الجوال
            وجدول على الديسكتوب، فيملك حاوية السحب الخاصة به. */}
        <div className="dashboard-panel mt-3 overflow-hidden p-0">
...
function Pagination({
  ...
  return (
    <nav className="mt-4 flex items-center justify-between gap-3">
      ...
      <span className="text-sm font-bold text-salon-charcoal">
